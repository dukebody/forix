...
class TestPriceExchange < Test::Unit::TestCase
  ...
  def test_target_currency_not_found
     price = Price.new(1, 'EUR')
     assert_raise(CurrencyNotFound) {@exchange.convert_to_currency(price, 'BTC')}
  end

  def test_origin_currency_not_found
     price = Price.new(1, 'BTC')
     assert_raise(CurrencyNotFound) {@exchange.convert_to_currency(price, 'EUR')}
  end
